...
const PomodoroTimerApp = () => {
  ...
  const { time, isRunning, start, pause, reset } = useTimer(settings.pomodoro * 60, (seconds) => {
    if (seconds === 0) {
      playSound();
    }
  });
  ...
  const handleCustomChange = (e) => {
    ...
    setCustomSettings({ ...customSettings, [name]: parseInt(value) || 0 });
  };

  const handleSaveSettings = () => {
    setSettings(customSettings);
    reset(customSettings[mode] * 60);
  };
      ...
      <main className="flex-grow flex justify-center items-center pt-10">
        <div className="w-full max-w-4xl mx-auto p-10 sm:p-12 md:p-16 lg:p-20 bg-white rounded-2xl shadow-lg text-center mt-10">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div>
              <label className="block text-sm text-gray-700 mb-1">Pomodoro (min)</label>
              <input
                type="number"
                name="pomodoro"
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-1">Short Break (min)</label>
              <input
                type="number"
                name="shortBreak"
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-1">Long Break (min)</label>
              <input
                type="number"
                name="longBreak"
          ...
          <button
            ...
            className="mb-4 px-4 py-2 bg-indigo-500 text-white rounded-lg shadow hover:bg-indigo-600"
          >
            Save Durations
          </button>
